...
Function GetBuildingName()
    SSProcess.ClearSelection
    SSProcess.ClearSelectCondition
    SSProcess.SetSelectCondition "SSObj_Code", "==", "9130511" '实测
    SSProcess.SelectFilter
        ...
        For j = 0 To llcount - 1
            For i = 0 To poicount - 1
                If LLarr(j,3) = SCarr(i,3) Then
                    ...
                    'MsgBox count
                    If Diffxy > 50 Then
                        g_docObj.SetCellText 4,row + k,10,"不符合",True,False
                        DifKzPoi(count) = SCarr(i,3)
                    Else
                        g_docObj.SetCellText 4,row + k,10,"符合",True,False
                    End If
                    If Diffh > 30 Then
                        g_docObj.SetCellText 4,row + k,11,"不符合",True,False
                        DifKzPoi(count) = SCarr(i,3)
                    Else
                        g_docObj.SetCellText 4,row + k,11,"符合",True,False
                    End If
                    k = k + 1
                    count = count + 1
                End If
