...
Sub OnClick()
    ...
    If  geocount > 0 Then
        ...
        If CODE = "9130235" Then pathName = fileName & "放样图.dwg" '项目名称+综合管线图.dwg
    End If
    SZDWT id,pathName
End Sub
...
Function SZDWT(TKID,fileName)
    ...
    SSProcess.SetDataXParameter "DataBoundMode", "5"'0(所有数据)， 1(选择集数据)， 2(当前图幅)， 3(缓冲区)，4(指定编码闭合地物)， 5(指定ID闭合地物)， 6(所有图幅)
    ...
    SSProcess.SetDataXParameter "ColorUseStatus", "0"       '颜色使用状态。0（按编码表设定颜色输出）、1（按地物设定颜色输出）
