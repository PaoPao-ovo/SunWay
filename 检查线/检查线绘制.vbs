...
Function SetLinepoiname(x1,y1,x2,y2,id)
    ...
    idstring = SSProcess.SearchNearObjIDs(x1,y1,0,0,"9130216",0) 
    ...
    idstring = SSProcess.SearchNearObjIDs(x2,y2,0,0,"9130311",0) 
    idarr = Split(idstring,",",-1,1) '与线上点相近的点的ids
    IdCount = UBound(idarr) + 1
    'MsgBox IdCount
...
Function DiffXY(id)
    ...
    If SelCount > 0 Then
        ...
        Y = transform(Y) 
        diffx = Abs(X - PointArr(1,0))  
        diffy = Abs(Y - PointArr(1,1))
