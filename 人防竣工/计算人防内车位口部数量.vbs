...
'添加代码
			...
			If geocount > 0 Then
				...
				'-------口部数量 9450053
					idskb = SSProcess.SearchInnerObjIDs(ID , 2,"9450053", 1)
					idsjd = SSProcess.SearchInnerObjIDs(ID , 2, "9460013,9460023,9460043,9460053", 1)
					SSFunc.ScanString idskb,",",kbsz,count1
						'msgbox count1
					if count1>=0 then  SSProcess.SetObjectAttr ID, "[kbsl]", count1
					SSFunc.ScanString idsjd,",",jdsz,count3
					if count3>=0 then  
						SSProcess.SetObjectAttr ID, "[tcwsl]", count3
						SSProcess.SetObjectAttr ID, "[CWGS]", 1
					End If 
				next 
			end if
			msgbox "统计完成"
