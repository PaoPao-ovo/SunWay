...
'检查模型名称
Dim CheckmodelName
CheckmodelName = "自定义脚本检查类->放线高程检查"

'检查描述
Dim strDescription
strDescription = "设计线高程为空"
...
Function ExportRecords(code)
    ...
    If SelCount > 0  Then
        ...
        For i = 0 To UBound(idarr)
            fxgc = SSProcess.GetObjectAttr(idarr(i),"[SheJGC]")
            ...
            fxgc = transform(fxgc)
            'MsgBox yxgc
            If fxgc = 0 Then SSProcess.AddCheckRecord strGroupName, strCheckName,CheckmodelName, strDescription, x, y, 0,0,idarr(i), ""
        Next 'i
    End If
